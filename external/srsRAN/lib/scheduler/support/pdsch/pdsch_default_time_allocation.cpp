  ...
  static constexpr std::array<pdsch_time_domain_resource_allocation, 16> table_normal_cp_dmrs3 = []() {
    ...
    for (unsigned row_index = 0; row_index <= 6; ++row_index) {
      if (row_index < 5) {
        ...
        table[row_index].symbols = {table[row_index].symbols.start() + 1, table[row_index].symbols.stop() + 1};
      } else {
        // row_index == 6.
        table[row_index].symbols = {table[row_index].symbols.start() + 2, table[row_index].symbols.stop() + 2};
      }
    }
    return table;
  }();
