...
public:
  ...
  /// Copy constructor.
  segmented_sib_list(const segmented_sib_list<T>& other) noexcept(std::is_nothrow_copy_constructible<T>::value) :
    buffers(other.buffers), current_segment(other.current_segment)
  {
  }
  ...
  segmented_sib_list<T>& operator=(const segmented_sib_list<T>& other)
  {
    ...
    current_segment = other.current_segment;
    return *this;
  }
