...
type Configuration struct {
	...
	DsmfTrigger            *DsmfTrigger      `yaml:"dsmfTrigger,omitempty" valid:"optional"`
}
...
func (c *Configuration) validate() (bool, error) {
	...
	if c.DsmfTrigger != nil {
		if _, err := c.DsmfTrigger.validate(); err != nil {
			return false, err
		}
	}
...
type DsmfTrigger struct {
	Enable             bool                  `yaml:"enable,omitempty" valid:"optional"`
	Uri                string                `yaml:"uri,omitempty" valid:"optional"`
	TimeoutSeconds     int                   `yaml:"timeoutSeconds,omitempty" valid:"optional"`
	TriggerEvent       string                `yaml:"triggerEvent,omitempty" valid:"optional"`
	ResultMode         string                `yaml:"resultMode,omitempty" valid:"optional,in(sync|async)"`
	TransportProtocol  string                `yaml:"transportProtocol,omitempty" valid:"optional"`
	PayloadProtocol    string                `yaml:"payloadProtocol,omitempty" valid:"optional"`
	OutputSchema       string                `yaml:"outputSchema,omitempty" valid:"optional"`
	TaskTimeoutSeconds uint32                `yaml:"taskTimeoutSeconds,omitempty" valid:"optional"`
	DataSource         *DsmfTriggerDataSource `yaml:"dataSource,omitempty" valid:"optional"`
	ProcessingSteps    []DsmfTriggerProcessingStep `yaml:"processingSteps,omitempty" valid:"optional"`
	Labels             map[string]string     `yaml:"labels,omitempty" valid:"optional"`
}

func (d *DsmfTrigger) validate() (bool, error) {
	if !d.Enable {
		return true, nil
	}

	var errs govalidator.Errors
	if d.Uri == "" || !govalidator.IsURL(d.Uri) {
		errs = append(errs, fmt.Errorf("invalid dsmfTrigger.uri: %q", d.Uri))
	}
	if d.TimeoutSeconds < 0 {
		errs = append(errs, fmt.Errorf("invalid dsmfTrigger.timeoutSeconds: %d", d.TimeoutSeconds))
	}
	if d.TriggerEvent != "" && d.TriggerEvent != "registration_request" && d.TriggerEvent != "pdu_session_establishment" {
		errs = append(errs, fmt.Errorf("unsupported dsmfTrigger.triggerEvent: %q", d.TriggerEvent))
	}
	if d.DataSource == nil {
		errs = append(errs, fmt.Errorf("dsmfTrigger.dataSource is required when enabled"))
	} else if _, err := d.DataSource.validate(); err != nil {
		errs = append(errs, err)
	}
	for _, step := range d.ProcessingSteps {
		if _, err := step.validate(); err != nil {
			errs = append(errs, err)
		}
	}
	if len(errs) > 0 {
...
type DsmfTriggerDataSource struct {
	SourceID         string              `yaml:"sourceId,omitempty" valid:"optional"`
	SourceCategory   string              `yaml:"sourceCategory,omitempty" valid:"optional"`
	SourceScenario   string              `yaml:"sourceScenario,omitempty" valid:"optional"`
	SourceTypeDetail string              `yaml:"sourceTypeDetail,omitempty" valid:"optional"`
	IngressEndpoint  DsmfTriggerEndpoint `yaml:"ingressEndpoint,omitempty" valid:"optional"`
	Encoding         string              `yaml:"encoding,omitempty" valid:"optional"`
	SubscribedTopics []string            `yaml:"subscribedTopics,omitempty" valid:"optional"`
	Attributes       map[string]string   `yaml:"attributes,omitempty" valid:"optional"`
}

func (d *DsmfTriggerDataSource) validate() (bool, error) {
	return d.IngressEndpoint.validate()
}

type DsmfTriggerEndpoint struct {
	Scheme string `yaml:"scheme,omitempty" valid:"optional"`
	Host   string `yaml:"host,omitempty" valid:"optional"`
	Port   uint32 `yaml:"port,omitempty" valid:"optional"`
	Path   string `yaml:"path,omitempty" valid:"optional"`
}

func (d DsmfTriggerEndpoint) validate() (bool, error) {
	if d.Scheme == "" {
		return false, fmt.Errorf("dsmfTrigger.dataSource.ingressEndpoint.scheme is required")
	}
	switch d.Scheme {
	case "file":
		if d.Path == "" {
			return false, fmt.Errorf("dsmfTrigger.dataSource.ingressEndpoint.path is required for file scheme")
		}
	case "http", "https":
		if d.Host == "" {
			return false, fmt.Errorf("dsmfTrigger.dataSource.ingressEndpoint.host is required for %s scheme", d.Scheme)
		}
	default:
		return false, fmt.Errorf("unsupported dsmfTrigger.dataSource.ingressEndpoint.scheme: %q", d.Scheme)
	}
	return true, nil
}

type DsmfTriggerProcessingStep struct {
	Name       string            `yaml:"name,omitempty" valid:"optional"`
	Version    string            `yaml:"version,omitempty" valid:"optional"`
	Parameters map[string]string `yaml:"parameters,omitempty" valid:"optional"`
}

func (d DsmfTriggerProcessingStep) validate() (bool, error) {
	if d.Name == "" {
		return false, fmt.Errorf("dsmfTrigger.processingSteps.name is required")
	}
	return true, nil
}
...
func (c *Config) GetDsmfTrigger() *DsmfTrigger {
	if c != nil && c.Configuration != nil {
		return c.Configuration.DsmfTrigger
	}
	return nil
}
