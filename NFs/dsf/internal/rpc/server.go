...
func New(addr string, proc *processor.Processor) *Server {
	s := grpc.NewServer()
	api.RegisterDsfControlServiceServer(s, proc)
	api.RegisterResultTransferServiceServer(s, proc)
	return &Server{
		addr:   addr,
		server: s,
	}
...
func (s *Server) Run(ctx context.Context, wg *sync.WaitGroup) error {
	...
	go func() {
		...
		logger.RPCLog.Infof("start DSF gRPC server on %s", s.addr)
		if err := s.server.Serve(lis); err != nil && !errors.Is(err, grpc.ErrServerStopped) {
			logger.RPCLog.Errorf("DSF gRPC server stopped with error: %v", err)
		}
	}()
	return nil
}
